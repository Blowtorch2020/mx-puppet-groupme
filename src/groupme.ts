...
export class GroupMe {
    ...
    async newPuppet(puppetId: number, data: any) {
        ...
        try {
            ...
            p.data.userId = String(me.id ?? me.user_id);
    ...
    async handleMatrixReadReceipt(
        ...
        // v2.groupme.com read_receipts may be deprecated; failures are non-fatal
        try {
    ...
    async createUser(user: IRemoteUser): Promise<IRemoteUser | null> {
        ...
        try {
            ...
            let profile: { name: string; avatarUrl: string } | undefined;
            const userIdStr = String(user.userId);
            // First try to get base profile from DMs (API may return other_user.id as number or string)
            const dm = dms.find(dm => String(dm.other_user.id) === userIdStr);
            if (dm) {
                profile = {
                ...
                const groupProfile = groups.flatMap(group => group.members || [])
                    .find(member => String(member.user_id) === userIdStr);
                if (groupProfile) {
                    profile = {
                        name: groupProfile.nickname ?? groupProfile.name,
            ...
            const roomOverrides = fromEntries(
                groups.flatMap(group => {
                    const groupProfile = (group.members || []).find(member => String(member.user_id) === userIdStr);
                    const groupId = group.id ?? group.group_id;

                    if (profile && groupProfile && groupId &&
